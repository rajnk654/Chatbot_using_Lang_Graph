...
def generate_chat_name():
    count = len(st.session_state["chat_threads"]) + 1
    return f"Chat {count}"
...
def reset_chat():
    ...
    add_thread(thread_id, chat_name)
...
if not st.session_state["chat_threads"]:
    first_id = st.session_state["thread_id"]
    add_thread(first_id, "Chat 1")
...
current_chat_name = st.session_state[
    "thread_names"
].get(thread_key, "Current Chat")
...
if uploaded_pdf:
    ...
    if uploaded_pdf.name in thread_docs:
        ...
        with st.sidebar.status(
            ...
            # Rename chat using PDF name
            st.session_state["thread_names"][
                thread_key
            ] = f"PDF: {uploaded_pdf.name}"
...
if not threads:
    ...
    for thread_id in threads:
        ...
        thread_name = st.session_state[
            "thread_names"
        ].get(thread_id)

        # If missing title → generate from history
        if not thread_name:
            ...
            if messages:
                ...
                if isinstance(first_msg, HumanMessage):
                    ...
                    thread_name = generate_title_from_message(
                        first_msg.content
                    )
